...
function WorkspaceRow({ workspace, busy }: { workspace: Workspace; busy: boolean }) {
...
export function WorkspaceOrderModal({ workspaces, onClose, onSaved }: {
  workspaces: Workspace[]; onClose: () => void; onSaved: (rows: Workspace[]) => void;
}) {
  ...
  return <Modal open onClose={() => { if (!busy) onClose(); }} title="Workspace 순서 설정">
    <div role="dialog" aria-label="Workspace 순서 설정" aria-modal="true" onKeyDown={e => { if (e.key === "Escape" && !busy && !e.defaultPrevented) onClose(); }}>
      <p className="workspace-order-help">손잡이를 끌어 Workspace 순서를 변경하세요.</p>
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={({ active, over }) => {
        ...
        <SortableContext items={rows.map(w => w.id)} strategy={verticalListSortingStrategy}>
          <ul className="workspace-order-list" aria-label="Workspace 순서">{rows.map(w => <WorkspaceRow key={w.id} workspace={w} busy={busy}/>)}</ul>
        </SortableContext>
      </DndContext>
