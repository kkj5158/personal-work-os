...
test("Every new-start action keeps old sessions and source-dialog double clicks create once", async () => {
  ...
  const keys=["quick-motivation","recovery","reality","present-life","grounded-future","past","review","sexual-pattern","responsibility"];
  ...
  try{
    ...
    assert.equal(sessions.filter(s=>s.status==='IN_PROGRESS').length,9);
  }finally{await act(()=>root.unmount());Object.assign(authoringApi,originals);dom.window.close();}
});
...
test("Home groups programs as Quick, Core and Topic from definition metadata and keeps a short recent list", async () => {
  ...
  const defs:[string,string][]=[["quick-motivation","QUICK"],["recovery","CORE"],["reality","CORE"],["present-life","CORE"],["grounded-future","CORE"],["past","CORE"],["review","CORE"],["sexual-pattern","TOPIC"],["responsibility","TOPIC"]];
  ...
  try{
    ...
    assert.deepEqual(groups,[["빠른 글쓰기",["quick-motivation"]],["핵심 글쓰기",["recovery","reality","present-life","grounded-future","past","review"]],["주제 글쓰기",["sexual-pattern","responsibility"]]]);
