...
export default async function Page({ params }: { params: Promise<{ program: string; sessionId: string; view?: string[] }> }) {
  ...
  if (!["quick-motivation", "recovery", "reality", "grounded-future", "past", "review", "sexual-pattern", "responsibility", "present-life"].includes(program) || !/^[\da-f-]{36}$/i.test(sessionId) || view.length > 1 || (view.length && !["full", "report"].includes(view[0]))) notFound();
