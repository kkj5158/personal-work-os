...
export default function AuthoringSession({ programKey, sessionId, mode }: { programKey: string; sessionId: string; mode: Mode }) {
  ...
  return <div ref={workspace} className="authoring-workspace" onCompositionStart={() => queue.current?.composition(true)} onCompositionEnd={() => queue.current?.composition(false)}>
    <header className="authoring-toolbar"><div><SystemSwitcher system="AUTHORING" compact /><Button type="button" variant="ghost" onClick={() => void go("/authoring")}>← Home</Button><strong>{definition.title}</strong></div><div>
      ...
      {session.sourceSessionId && <Button type="button" onClick={() => void openReference()}>{session.programKey === "review" ? "원본 Report 보기" : "Reality 보기"}</Button>}
    ...
    {reference && <AuthoringDialog title="완료 기록 참고" onClose={() => setReference(null)}><p>{reference.definition.title} · {reference.completedAt?.slice(0, 10)}</p><ReportDocument session={reference} /></AuthoringDialog>}
