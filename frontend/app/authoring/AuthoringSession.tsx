...
import { hasAnswer, sessionRoute, type Answer, type Draft, type Session } from "@/lib/authoring/types";
...
export default function AuthoringSession({ programKey, sessionId, mode }: { programKey: string; sessionId: string; mode: Mode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        current.current = loaded; setSession(loaded); setTitle?.(loaded.definition.title);
        ...
        const saver = new Autosave<Draft>(async draft => {
          ...
          const result = await authoringApi.save(sessionId, latest.version, draft);
          ...
          if (!cancelled) remember(sessionId, { answers: next.answers, currentSectionKey: next.currentSectionKey, version: next.version });
        }, (state, failure) => {
        ...
        if (draftDiffers(draft, loaded)) {
          ...
          if (draft.version === loaded.version) saver.set({ answers: restored.answers, currentSectionKey: restored.currentSectionKey });
  ...
  function change(answerKey: string, answer: Answer) {
    ...
    const draft = { answers: next.answers, currentSectionKey: next.currentSectionKey };
  ...
  async function section(key: string) {
    ...
    if (next.status === "IN_PROGRESS") {
      const draft = { answers: next.answers, currentSectionKey: key };
      remember(sessionId, { ...draft, version: next.version });
      if (!conflict) queue.current?.set(draft);
    }
  ...
  if (!session) return <div className="authoring-loading"><SystemSwitcher system="AUTHORING" />{error ? <div role="alert"><p>{error}</p><Button onClick={() => location.reload()}>다시 시도</Button><Button onClick={() => void go("/authoring")}>Home</Button></div> : <p role="status">세션을 불러오는 중…</p>}</div>;
  ...
  const missing = definition.sections.flatMap(s => s.questions).filter(q => definition.completionKeys.includes(q.questionKey) && !hasAnswer(session.answers[q.questionKey]));
  ...
  return <div ref={workspace} className="authoring-workspace" onCompositionStart={() => queue.current?.composition(true)} onCompositionEnd={() => queue.current?.composition(false)}>
    <header className="authoring-toolbar"><div><SystemSwitcher system="AUTHORING" compact /><Button variant="ghost" onClick={() => void go("/authoring")}>← Home</Button><strong>{definition.title}</strong></div><div>
      ...
      {session.sourceSessionId && <Button onClick={() => void openReference()}>Reality 보기</Button>}
      <Button onClick={() => void go(sessionRoute(session, mode === "full" && editable ? "" : "full"))}>{mode === "full" && editable ? "작성으로 돌아가기" : "전체 내용 보기"}</Button>
      {!editable && mode !== "report" && <Button onClick={() => void go(sessionRoute(session, "report"))}>Report 보기</Button>}
    </div></header>
    {error && <div role="alert" className="authoring-error">{error}{editable && <Button onClick={() => void retry()}>저장 다시 시도</Button>}</div>}
    {retainedDraft && <div role="status" className="authoring-error">다른 화면에서 세션이 완료되었습니다. 이 탭의 미저장 입력은 따로 보관되어 있습니다. <Button onClick={() => setShowRetainedDraft(true)}>미저장 입력 보기</Button></div>}
    <div className={`authoring-workspace-body ${isDocument ? "document-mode" : ""}`}>
      {!isDocument && <nav className="authoring-sections" aria-label="프로그램 섹션"><p>{index + 1} / {definition.sections.length}</p>{definition.sections.map((s, i) => <button key={s.sectionKey} type="button" aria-current={i === index ? "step" : undefined} onClick={() => void section(s.sectionKey)}><span>{String(i + 1).padStart(2, "0")}</span><span>{s.title}</span>{s.questions.length > 0 && s.questions.every(q => hasAnswer(session.answers[q.questionKey])) && <span aria-label="작성됨">✓</span>}</button>)}</nav>}
      <main className="authoring-canvas">
        {isDocument ? <><p className="authoring-eyebrow">{editable ? "REVIEW" : "COMPLETED"} · {session.specVersion}</p><h1>{mode === "report" ? `${definition.title} Report` : "전체 내용 보기"}</h1><p className="authoring-muted">{definition.title}</p>
          {mode === "report" ? <><ReportDocument session={session} />{session.programKey === "recovery" && <div className="authoring-export"><Button disabled title="OPS Recovery Protocol API 연결 대기">OPS Recovery Protocol에 적용</Button><p>OPS 연결 대기 · Authoring의 구조화 결과를 파일로 보관할 수 있습니다.</p><Button onClick={() => void exportRecovery()}>구조화 결과 다운로드 (JSON)</Button></div>}</> : <SessionDocument session={session} edit={editable ? key => void section(key) : undefined} />}
          {editable && <footer className="authoring-navigation"><Button onClick={() => void go(sessionRoute(session))}>계속 작성</Button><Button variant="primary" onClick={() => { setAcknowledged(false); setCompletion(true); }}>세션 완료 검토 →</Button></footer>}
          ...
          {index === 0 && definition.guidance && <details className="authoring-source"><summary>프로그램 안내</summary><p>{definition.guidance}</p></details>}<div className="authoring-questions">{active.questions.map(q => <QuestionField key={q.questionKey} question={q} value={session.answers[q.questionKey]} answers={session.answers} change={value => change(q.questionKey, value)} />)}</div>
