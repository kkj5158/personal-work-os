...
export function QuestionField({ question: q, value, answers, change }: { question: Question; value?: Answer; answers: Answers; change: (value: Answer) => void }) {
  ...
  return <fieldset className={`authoring-question type-${q.type}`} id={q.questionKey}>
    ...
    {q.type === "FREE_TEXT" && <textarea id={id} aria-label={q.prompt} aria-describedby={hint} value={typeof value === "string" ? value : ""} rows={q.helperText && q.helperText.length > 200 ? 12 : 5} onChange={e => change(e.target.value)} />}
...
function ClassificationField({ question: q, value, source, change }: { question: Question; value: Classification[]; source?: Answer; change: (value: Answer) => void }) {
  ...
  return <div className="authoring-classification">
    {sourceLines.length > 0 && <details className="authoring-source"><summary>UNLOAD 다시 보기</summary><p>{String(source)}</p><Button disabled={!remaining.length} onClick={() => change([...value, ...remaining.map(text => ({ text, classification: "", timing: "" }))])}>줄별로 항목 가져오기</Button></details>}
    {value.map((row, i) => <div className="authoring-classification-row" key={i}>
      ...
      <Button variant="ghost" aria-label={`항목 ${i + 1} 삭제`} onClick={() => change(value.filter((_, n) => n !== i))}>×</Button>
    </div>)}
    <Button onClick={() => change([...value, { text: "", classification: "", timing: "" }])}>+ 항목 추가</Button>
  </div>;
}
export function AnswerValue({ value, type }: { value?: Answer; type: Question["type"] }) {
  ...
  if (!hasAnswer(value)) return <p className="authoring-empty">아직 작성하지 않음</p>;
