...
import type { Answer, Epoch, Experience, Goal, QuestionType } from "@/lib/authoring/types";

export const emptyEpochs = (): Epoch[] => Array.from({length:7}, (_,i) => ({id:`epoch-${i+1}`,title:"",experiences:[]}));
export const emptyGoal = (): Goal => ({id:crypto.randomUUID(),title:"",description:"",why:"",impact:"",strategy:"",obstacles:"",benchmark:""});
const deepDive = [
...
export function GoalsWriting({type,value,change,guides}:{type:QuestionType;value?:Answer;change:(value:Answer)=>void;guides?:Record<string,string>}) {
  ...
  return <div className="authoring-structured"><p className="authoring-muted">목표 {goals.length} / 6–8개 · 순서를 바꿔도 작성한 분석은 함께 보존됩니다.</p>
    {goals.map((goal,index)=><section className="authoring-writing-card" key={goal.id}>
      ...
      {type === "GOALS" ? <>
        ...
        <label className="authoring-field">목표 {index+1} 설명<textarea rows={5} value={goal.description} onChange={e=>update(goal.id,{description:e.target.value})} /></label>
        <div className="authoring-row-actions"><Button type="button" disabled={index===0} onClick={()=>move(index,-1)} aria-label={`목표 ${index+1} 위로`}>↑ 위로</Button><Button type="button" disabled={index===goals.length-1} onClick={()=>move(index,1)} aria-label={`목표 ${index+1} 아래로`}>↓ 아래로</Button><Button type="button" variant="ghost" onClick={()=>{if(window.confirm("이 목표와 연결된 분석을 삭제할까요?"))change(goals.filter(g=>g.id!==goal.id));}}>목표 {index+1} 삭제</Button></div>
      </> : <>
    ...
    {type === "GOALS" ? <Button type="button" disabled={goals.length>=8} onClick={()=>change([...goals,emptyGoal()])}>+ 목표 추가</Button> : !goals.length && <p>GOALS에서 목표를 먼저 작성하세요.</p>}
  </div>;
}
export function PastWriting({type,value,change}:{type:QuestionType;value?:Answer;change:(value:Answer)=>void}) {
      ...
      {type === "EPOCHS" ? <label className="authoring-field">Epoch {index+1} 제목<input value={epoch.title} onChange={e=>update(epoch.id,{title:e.target.value})} /></label> : <>
        ...
        {epoch.experiences.map((experience,n)=><div className="authoring-experience" key={experience.id}>
          {type === "EXPERIENCES" ? <>
            ...
            <label className="authoring-field">무슨 일이 있었나요?<span className="authoring-help">사건 자체를 충분히 적으세요. 이유와 현재에 미친 영향은 다음 단계에서 살펴봅니다.</span><textarea rows={10} aria-label={`Epoch ${index+1} 경험 ${n+1} 사건`} value={experience.event} onChange={e=>updateExperience(epoch,experience.id,{event:e.target.value})} /></label>
            ...
            <label className="authoring-field">이 경험은 당신의 삶을 어떻게 형성했고, 지금의 당신을 만드는 데 어떤 영향을 주었나요?<span className="authoring-help">다른 사람을 바라보는 방식, 세상을 바라보는 방식에 미친 영향을 함께 적어보세요.</span><textarea rows={10} aria-label={`Epoch ${index+1} 경험 ${n+1} 영향`} value={experience.effects} onChange={e=>updateExperience(epoch,experience.id,{effects:e.target.value})} /></label>
...
export function StructuredAnswer({value,type}:{value?:Answer;type:QuestionType}) {
  ...
  if (type === "GOALS" || type === "GOAL_DEEP_DIVE") return <div>{(value as Goal[]).map((goal,index)=><section className="authoring-writing-card" key={goal.id}><h4>{index+1}. {goal.title || "제목 미작성"}</h4><p className="authoring-answer">{goal.description}</p>{type === "GOAL_DEEP_DIVE" && deepDive.map(field=><div key={field.key}><strong>{field.title}</strong><p className="authoring-answer">{goal[field.key] || "아직 작성하지 않음"}</p></div>)}</section>)}</div>;
