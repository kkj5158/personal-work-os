...
import { answerFor, hasAnswer, programTitle, scanSummary, sectionLabel, type ReportItem, type Session } from "@/lib/authoring/types";
...
export function SessionDocument({ session, edit }: { session: Session; edit?: (key: string) => void }) {
  return <div className="authoring-document">{session.definition.guidance && <p className="authoring-help">{session.definition.guidance}</p>}{session.definition.sections.map((section, i) => <section key={section.sectionKey}>
    ...
    {section.prompt && <p className="authoring-section-prompt">{section.prompt}</p>}{section.description && <p className="authoring-help">{section.description}</p>}
...
export function ReportDocument({ session }: { session: Session }) {
  ...
  // Optional compact values appear in a report only when they were written; nothing is filled in for them.
  const questions = new Map(session.definition.sections.flatMap(s => s.questions).map(q => [q.questionKey, q]));
  const shown = (item: ReportItem) => !questions.get(item.questionKey)?.metadata?.omitWhenEmpty || hasAnswer(item.value);
  return <div className="authoring-document"><section><h2>Session Summary</h2>{source && <p className="authoring-muted">원본: {source.programKey} · {new Date(source.completedAt).toLocaleDateString("ko-KR",{timeZone:"Asia/Seoul"})} · {source.specVersion}</p>}<p>{programTitle(session)} · {session.specVersion}</p><p>{new Date(session.report.completedAt).toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}</p>{summaryItems.map(item => <div key={item.questionKey}><h3>{item.prompt}</h3><AnswerValue value={item.value} type={item.type} /></div>)}{scan && <div className="authoring-scan-summary"><p>평균 {scan.average.toFixed(1)} / 10 · {scan.count}개 작성</p><p>상위 안정 영역 · {scan.highest.map(s => `${s.prompt} ${s.value}`).join(" · ")}</p><p>하위 영역 · {scan.lowest.map(s => `${s.prompt} ${s.value}`).join(" · ")}</p>{session.programKey === "reality" && <p>최고–최저 점수 차이 · {scan.spread}</p>}</div>}</section>{session.report.sections.map((section, index) => <section key={index}><h2>{section.title}</h2>{section.items.filter(shown).map(item => <div key={item.questionKey} className="authoring-document-question"><h3>{item.prompt}</h3><AnswerValue value={item.value} type={item.type} /></div>)}</section>)}</div>;
}
