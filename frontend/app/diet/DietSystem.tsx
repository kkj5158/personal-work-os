...
/** The existing NOTE SYS "다이어트 기록" workspace: stable id from the DIET NOTE-sync setting, else NOTE SYS's own name resolution (never creates a workspace). */
export const DIET_NOTE_WORKSPACE="다이어트 기록";
export const dietNoteHref=(workspaceId:string|null|undefined)=>workspaceId?`/notes?workspace=${workspaceId}`:`/notes?workspaceName=${encodeURIComponent(DIET_NOTE_WORKSPACE)}`;
...
export default function DietSystem(){const store=useDietStore(),path=usePathname(),router=useRouter(),shell=useGlobalTabs();const navigate=(href:string)=>shell?shell.navigate(href):router.push(href);
 ...
 useEffect(()=>{let live=true;dailyNotesApi.syncSettings().then(s=>{if(live)setNoteWorkspaceId(s.workspaceId);}).catch(()=>{});return()=>{live=false;};},[]);
