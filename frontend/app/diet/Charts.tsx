...
type Series = { id?: string; name: string; color: string; values: (number | null)[]; dashed?: boolean; connectGaps?: boolean; targetTrend?: boolean };
...
export function SeriesChart({ dates, series, lines = [], bands = [], unit = "", bar = false, markers = [] }: {
  ...
  return <div className="diet-series-chart">
    <div className="diet-chart-legend">
      {series.map(s => <span key={s.id ?? s.name}><i style={s.targetTrend ? { width: 18, height: 0, borderTop: `2px ${s.dashed ? "dashed" : "solid"} ${s.color}`, borderRadius: 0 } : { background: s.color }} />{s.name}</span>)}
    ...
    {dates.length === 0 || extent.length === 0 ? <div className="diet-chart-empty">선택한 기간에 표시할 기록이 없습니다.</div> : <>
      <div className="diet-chart-scroll">
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${series.map(s => s.name).join(", ")} 추이 (${unit})`}>
          ...
          {series.map((s, si) => <g key={s.id ?? s.name}>
      ...
      <div className="diet-chart-readout" aria-live="polite">{activeIndex != null ? <><strong>{dates[activeIndex]}</strong>{series.map(s => <span key={s.id ?? s.name} style={{ color: s.color }}>{s.name} <b>{s.values[activeIndex] == null ? "—" : `${fmt(s.values[activeIndex]!)} ${unit}`}</b></span>)}{plottedMarkers.filter(m => m.date === dates[activeIndex]).map(m => <span key={m.id}>◇ {m.label} {fmt(m.value)} {unit}</span>)}</> : <span>그래프에 마우스를 올리거나 날짜를 선택해 값을 확인하세요.</span>}</div>
    </>}
  </div>;
}
