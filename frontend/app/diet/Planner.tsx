...
import type { Challenge, DietStore, Importance, Milestone, WeightGoal } from "@/lib/diet/types";
...
const goalNames = { SHORT_TERM: "단기 목표", WEEKLY: "주 목표", MONTHLY: "월 목표", FINAL: "최종 목표" };
...
function newChallenge(order: number): Challenge {
  return { id: crypto.randomUUID(), title: "", type: "WEIGHT", status: "ACTIVE", startDate: today(), endDate: addDays(today(), 27), color: "#3b82f6", keyPoint: "", notes: [], sortOrder: order, startWeight: null, targetWeight: null, itemIds: [], goalMode: "RATE", includeMissing: true, currentValue: null, targetValue: null };
}
...
export default function Planner({ store }: { store: DietStore }) {
  ...
  const weightChallenges = challenges.filter(c => c.type === "WEIGHT" && (status === "ALL" || c.status === status));
  ...
  const current = [...data.days].filter(d => d.date <= today() && d.morningWeight != null).sort((a, b) => b.date.localeCompare(a.date))[0]?.morningWeight;
  ...
  return <div className="diet-planner">
    <div className="dp-toolbar">
      <div className="dp-tabs" aria-label="챌린지 상태">
        {(["ACTIVE", "WAITING", "COMPLETED", "STOPPED", "ALL"] as const).map(s => <button key={s} aria-pressed={status === s} onClick={() => setStatus(s)}>{s === "ALL" ? "전체" : statusNames[s]}</button>)}
      </div>
      ...
      <section className="dp-panel dp-calendar" aria-label="체중 계획 달력">
        ...
        {weeks.map(week => <div className="dp-week" key={week[0]}>
          <div className="dp-calendar-days">{week.map(date => {
            const goals = data.goals.filter(g => g.date === date && (!g.challengeId || weightChallenges.some(c => c.id === g.challengeId)));
            const milestones = data.milestones.filter(m => m.date === date && weightChallenges.some(c => c.id === m.challengeId));
            const dailyTarget = data.days.find(d => d.date === date)?.targetWeight;
            const finals = weightChallenges.filter(c => c.endDate === date && c.targetWeight != null && !goals.some(g => g.challengeId === c.id && g.value === c.targetWeight));
            return <div key={date} className={`dp-date ${date.slice(0, 7) !== month.slice(0, 7) ? "outside" : ""} ${date === today() ? "today" : ""}`}>
              ...
              {dailyTarget != null && <div className="dp-target" title="일별 목표">{format(dailyTarget, " kg")}</div>}
              {goals.map(g => <div key={g.id} className="dp-target" style={{ color: challenges.find(c => c.id === g.challengeId)?.color }} title={goalNames[g.kind]}>{format(g.value, " kg")}</div>)}
              {milestones.map(m => <div key={m.id} className="dp-target" style={{ color: challenges.find(c => c.id === m.challengeId)?.color }} title={m.title || "마일스톤"}>{format(m.value, " kg")}</div>)}
              {finals.map(c => <div key={c.id} className="dp-target" style={{ color: c.color }} title="최종 목표">{format(c.targetWeight, " kg")}</div>)}
              {date === today() && current != null && <div className="dp-actual">현재 {format(current, " kg")}</div>}
            </div>;
          })}</div>
          <div className="dp-duration-lines">{weightChallenges.filter(c => c.startDate <= week[6] && c.endDate >= week[0]).map(c => {
            const start = week.findIndex(d => d >= c.startDate);
            const end = week.filter(d => d <= c.endDate).length;
            return <div key={c.id} className="dp-duration-row"><button title={`${c.title} · ${c.startDate} – ${c.endDate}`} aria-label={`${c.title} 기간`} style={{ gridColumn: `${start + 1} / ${end + 1}`, background: c.color }} onClick={() => setSelectedId(c.id)}>{c.title}</button></div>;
          })}</div>
        </div>)}
        <p className="dp-hint">목표 체중 · 체중 챌린지 기간 · 오늘의 현재 체중</p>
      </section>

      <section className="dp-panel dp-timeline"><h2>타임라인</h2>
        {selected?.type === "WEIGHT" ? <>
          <p className="dp-hint">{selected.title}</p>
          <ol style={{ borderColor: selected.color }}>
            {[{ id: "start", date: selected.startDate, title: "시작", value: selected.startWeight },
              ...data.goals.filter(g => g.challengeId === selected.id && g.kind !== "FINAL").map(g => ({ ...g, title: goalNames[g.kind] })),
              ...selectedMilestones.map(m => ({ ...m, title: m.title || "마일스톤" })),
              { id: "final", date: selected.endDate, title: "최종 목표", value: selected.targetWeight }]
              .sort((a, b) => a.date.localeCompare(b.date)).map(point => <li key={point.id} style={{ color: selected.color }}><div><strong>{point.title}</strong><time>{point.date}</time></div><b>{format(point.value, " kg")}</b></li>)}
          </ol>
        </> : <p className="dp-empty">체중 챌린지를 선택하면 시작부터 최종 목표까지 표시됩니다.</p>}
      </section>
    </div>

    <div className="dp-bottom">
      <section className="dp-panel dp-challenges"><h2>챌린지 목록 <small>{visible.length}</small></h2>
      ...
      <section className="dp-panel dp-detail">
        {!selected ? <p className="dp-empty">챌린지를 추가하면 목표와 마일스톤을 관리할 수 있습니다.</p> : <>
          <div className="dp-toolbar"><div><h2 style={{ borderLeft: `4px solid ${selected.color}`, paddingLeft: 10 }}>{selected.title}</h2><p className="dp-hint">{typeNames[selected.type]} · {selected.startDate} – {selected.endDate} · {statusNames[selected.status]}</p></div><button onClick={() => setEditing({ ...selected, itemIds: [...selected.itemIds], notes: [...selected.notes] })}>수정</button></div>
          ...
          <div className="dp-metrics">
            {selected.type === "WEIGHT" ? <><Metric label="시작 체중" value={format(selected.startWeight, " kg")} /><Metric label="현재 체중" value={format(progress?.current, " kg")} /><Metric label="감량" value={format(progress?.lost, " kg")} loss={(progress?.lost ?? 0) > 0} /><Metric label="남은 감량" value={format(progress?.remaining, " kg")} /></> : <><Metric label={selected.type === "CHECKLIST" ? (selected.goalMode === "RATE" ? "현재 성공률" : "성공 횟수") : "현재 값"} value={format(progress?.current, selected.type === "CHECKLIST" && selected.goalMode === "RATE" ? "%" : "")} /><Metric label="목표" value={format(progress?.target, selected.type === "CHECKLIST" && selected.goalMode === "RATE" ? "%" : "")} /></>}
...
function GoalEditor({ store, challenge, kind }: { store: DietStore; challenge: Challenge; kind: WeightGoal["kind"] }) {
  const existing = store.data.goals.find(g => g.challengeId === challenge.id && g.kind === kind);
  const [value, setValue] = useState(existing?.value.toString() ?? (kind === "FINAL" ? challenge.targetWeight?.toString() ?? "" : ""));
  const [date, setDate] = useState(existing?.date ?? challenge.endDate);
...
function ChallengeEditor({ initial, store, onClose, onSaved }: { initial: Challenge; store: DietStore; onClose: () => void; onSaved: (id: string) => void }) {
  ...
  return <div className="dp-modal-backdrop" onClick={onClose}><section className="dp-modal" role="dialog" aria-modal="true" aria-labelledby="dp-challenge-title" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === "Escape") onClose(); }}>
    ...
    <form onSubmit={async e => { e.preventDefault(); setError(""); if (draft.endDate < draft.startDate) { setError("종료일은 시작일 이후여야 합니다."); return; } if (draft.type === "CHECKLIST" && !draft.itemIds.length) { setError("체크리스트 항목을 하나 이상 선택해 주세요."); return; } setSaving(true); try {
      ...
      <label>핵심<textarea rows={2} value={draft.keyPoint} onChange={e => change("keyPoint", e.target.value)} placeholder="짧은 핵심 문장" /></label><label>메모<textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="한 줄에 하나씩 입력하면 글머리 목록으로 표시됩니다." /></label>
...
function MilestoneEditor({ initial, store, onClose }: { initial: Milestone; store: DietStore; onClose: () => void }) {
  ...
  const submit = async (remove = false) => { setError(""); setSaving(true); try { if (remove) await store.remove("milestones", draft.id); else await store.save("milestones", draft); onClose(); } catch (e) { setError(e instanceof Error ? e.message : "저장 실패"); } finally { setSaving(false); } };
  return <div className="dp-modal-backdrop" onClick={onClose}><section className="dp-modal dp-modal-small" role="dialog" aria-modal="true" aria-labelledby="dp-milestone-title" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === "Escape") onClose(); }}><div className="dp-toolbar"><h2 id="dp-milestone-title">마일스톤 {existing ? "수정" : "추가"}</h2><button onClick={onClose} aria-label="닫기">×</button></div><form onSubmit={e => { e.preventDefault(); void submit(); }}>
    <label>날짜<input autoFocus type="date" min={challenge?.startDate} max={challenge?.endDate} required value={draft.date} onChange={e => setDraft({ ...draft, date: e.target.value })} /></label><label>목표 값 {challenge?.type === "WEIGHT" ? "(kg)" : ""}<input type="number" step="any" min={challenge?.type === "WEIGHT" ? "0.1" : "0"} required value={draft.value} onChange={e => setDraft({ ...draft, value: Number(e.target.value) })} /></label><label>제목 (선택)<input value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} /></label><label>메모 (선택)<textarea rows={3} value={draft.memo} onChange={e => setDraft({ ...draft, memo: e.target.value })} /></label>
