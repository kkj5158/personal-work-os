...
export default function GlobalGoals({ store }: { store: DietStore }) {
  ...
  const add = (kind: WeightGoal["kind"]) => edit({ id: crypto.randomUUID(), kind, baselineDate: today(), baselineWeight: null, targetDate: today(), targetWeight: goalFor(store.data, kind)?.targetWeight ?? 0, core: "", memoItems: [] });
  return <section className="dp-panel dp-global-goals" aria-label="공유 체중 목표 관리">
    ...
    {draft && <div className="dp-modal-backdrop" onClick={() => setDraft(null)}><section className="dp-modal dp-modal-small" role="dialog" aria-modal="true" aria-labelledby="dp-global-title" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === "Escape") setDraft(null); }}>
      ...
      <form onSubmit={async e => { e.preventDefault(); setError(""); setSaving(true); try { const fields = new FormData(e.currentTarget); await store.save("goals", { ...draft, targetDate: String(fields.get("targetDate")), core: draft.core.trim(), memoItems: draft.memoItems.map(item => item.trim()).filter(Boolean) }); setDraft(null); } catch (e) { setError(e instanceof Error ? e.message : "목표 저장 실패"); } finally { setSaving(false); } }}>
        ...
        <label>기준일<input type="date" required={draft.baselineWeight != null} value={draft.baselineDate ?? ""} onChange={e => setDraft({ ...draft, baselineDate: e.target.value || null })} /></label>
        <label>기준 체중 (kg)<input type="number" min="0.1" step="0.1" required={!!draft.baselineDate} value={draft.baselineWeight ?? ""} onChange={e => setDraft({ ...draft, baselineWeight: e.target.value === "" ? null : Number(e.target.value) })} /></label>
        <small>기준일과 기준 체중을 저장하면 목표일까지의 계획선을 표시합니다. 실제 기록이 바뀌어도 기준은 유지됩니다.</small>
        <label>목표 날짜<input name="targetDate" required type="date" min={draft.baselineDate ? new Date(Date.parse(`${draft.baselineDate}T00:00:00Z`) + 86400000).toISOString().slice(0,10) : undefined} defaultValue={draft.targetDate} /></label>
