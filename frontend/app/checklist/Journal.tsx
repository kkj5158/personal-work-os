...
import { availabilityOf, journalGroups, periodsByItem, ALL_IMPORTANCE } from "@/lib/checklist-sys/model";
...
/** Journal — the default operational surface: Identity/Area-grouped item rows × date columns. */
export default function Journal({ store, scope, onClearScope, navigate }: { store: ChecklistSysStore; scope: Scope; onClearScope: (keepIdentity: boolean) => void; navigate: (href: string) => void }) {
  ...
  const groups: GridGroup[] = useMemo(() => sections.map(({ identity: owner, area: section, items }) => ({
    ...
    rows: items.map(item => ({ id: item.id, label: item.name, icon: item.icon })),
  ...
  return (
    <>
      ...
      <p className="cks-hint">클릭 = 성공 · 다시 클릭 = 초기화 · 우클릭/1·2·3·0 키 = 상태 선택 · 드래그 = 여러 칸 선택 · 날짜 머리글 = 날짜 전체 기록 못함</p>
      {catalog.areas.length > 0 && <button type="button" className="cks-add" onClick={() => setCreating(true)}><Plus size={14} />체크리스트 추가</button>}
      {creating && <ItemDrawer store={store} initialAreaId={scope.areaId ?? catalog.areas.find(a => !scope.identityId || a.identityId === scope.identityId)?.id} onClose={() => setCreating(false)} />}
    </>
  );
}
