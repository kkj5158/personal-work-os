...
import { TITLE_KEY,PRESET_KEY,RECENT_BLOCK_KEY,readQuickBlocks,rememberQuickBlock,titleSuggestions,presetPatch,type TitleUse,type CalendarPreset } from "./creationPresets";
import { formatDuration } from "./duration";
import { actualAllowed, futureActualMessage } from "./actualPolicy";
import { STATE_LABELS } from "./statePolicy";

export function CalendarEditor({ value, date, categories, status, error, guard, busy, onChange, onSave, onFlush, onDelete, onClose, onDiscard, onContinue, presentationColor="", onPresetColor, onStateChange, transitioning=false }: {
  onStateChange?:(kind:"plan"|"actual")=>void;transitioning?:boolean;
  ...
  const [titles,setTitles]=useState<TitleUse[]>([]),[presets,setPresets]=useState<CalendarPreset[]>([]),[recent,setRecent]=useState<CalendarPreset[]>([]);
  // Hydrate browser-only history/preferences after SSR and selection changes.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(()=>{try {setTitles(JSON.parse(localStorage.getItem(TITLE_KEY) ?? "[]"));setPresets(readQuickBlocks(localStorage.getItem(PRESET_KEY)));setRecent(readQuickBlocks(localStorage.getItem(RECENT_BLOCK_KEY)));}catch{}},[value?.key]);
  function saveQuickBlock(){if(!value?.title.trim())return;const next=[...presets,{id:crypto.randomUUID(),title:value.title,domainType:value.domainType,categoryId:value.categoryId,duration:value.unscheduled?value.duration:timeMinutes(value.end)-timeMinutes(value.start),color:presentationColor}];setPresets(next);try{localStorage.setItem(PRESET_KEY,JSON.stringify(next));}catch{}}
  ...
  return <aside className="calendar-editor" aria-label="일정 편집기">
    <header><span>{value ? ({plan:"Plan",actual:"Actual",state:"State · LIFE CODE"}[value.kind]) : "일정 상세"}</span><button aria-label="편집기 접기" onClick={onClose}><PanelRightClose size={17}/></button></header>
    {guard && <div className="cal-unsaved-guard" role="alert"><p>저장되지 않은 {value?.kind === "state" ? "상태" : value?.kind === "plan" ? "계획" : "Actual"} 기록이 있습니다.</p><button onClick={onDiscard}>변경사항 버리기</button><button onClick={onContinue}>계속 편집</button></div>}
    {!value ? <div className="cal-editor-empty"><CalendarDays size={27}/><h2>{date}</h2><p>빈 시간을 클릭하거나 드래그해 일정을 만드세요.</p><p>기존 블록을 클릭하면 여기에서 편집할 수 있습니다.</p></div> : <fieldset disabled={transitioning} className="cal-editor-fields" onBlur={onFlush} onKeyDown={e => { if(e.key === "Enter" && e.target instanceof HTMLInputElement && value.kind !== "state") { e.preventDefault(); onFlush(); } }}>
      {value.kind !== "state" && <div className="cal-state-control"><div role="group" aria-label="블록 상태" className="cal-state-selector">{(["plan","actual"] as const).map(kind=><button type="button" key={kind} aria-pressed={value.kind===kind} disabled={transitioning || kind==="actual" && !actualAllowed(value.date)} aria-describedby={kind==="actual" && !actualAllowed(value.date) ? "calendar-actual-date-rule" : undefined} onClick={()=>onStateChange ? onStateChange(kind) : onChange({kind})}>{kind==="plan" ? "Plan" : "Actual"}</button>)}</div>{!actualAllowed(value.date) && <p id="calendar-actual-date-rule" className="cal-state-explanation">{futureActualMessage}</p>}</div>}
      ...
      {value.kind!=="state" && <><datalist id="calendar-title-suggestions">{titleSuggestions(titles,value.title).map(t=><option key={t} value={t}/>)}</datalist><div className="cal-presets"><label>빠른 블록<select aria-label="빠른 블록" value="" onChange={e=>{const p=[...presets,...recent].find(p=>p.id===e.target.value);if(p){const patch=presetPatch(p,value);if(value.kind==="actual" && value.id && p.domainType!==value.domainType)return;onChange(patch);if(p.color)onPresetColor?.(p.domainType,p.categoryId,p.color);rememberQuickBlock({...value,...patch},p.color);setRecent(readQuickBlocks(localStorage.getItem(RECENT_BLOCK_KEY)));}}}><option value="">선택하면 바로 적용</option>{recent.length>0 && <optgroup label="최근 사용">{recent.map(p=><option key={p.id} value={p.id} disabled={value.kind==="actual" && !!value.id && p.domainType!==value.domainType}>{p.title} · {formatDuration(p.duration)}</option>)}</optgroup>}<optgroup label="저장한 블록">{presets.map(p=><option key={p.id} value={p.id} disabled={value.kind==="actual" && !!value.id && p.domainType!==value.domainType}>{p.title} · {formatDuration(p.duration)}</option>)}</optgroup></select></label><button type="button" disabled={!value.title.trim()} onClick={saveQuickBlock}>+ 빠른 블록으로 저장</button></div></>}
      ...
      {value.unscheduled && value.kind === "plan" ? <p>시간 미지정 · 기본 {formatDuration(value.duration)}</p> : value.unscheduled && value.kind === "actual" ? <label>소요 시간 (분)<input type="number" min="1" value={value.duration} onChange={e => onChange({duration:Number(e.target.value)})}/></label> : <p className="cal-duration">{Number.isFinite(duration) ? formatDuration(duration) : "시간을 입력하세요"}</p>}
      {value.kind !== "state" ? <>
