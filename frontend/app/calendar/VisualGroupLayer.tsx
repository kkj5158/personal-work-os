...
export interface VisualGroupLayerProps {
  groups: CalendarVisualGroup[]; date?: string; dates?: string[]; scale: number; selectedId?: string;
...
export function VisualGroupLayer({ groups, date, dates: range, scale, selectedId, onSelect, onPointerDown }: VisualGroupLayerProps) {
  ...
  return <div className="cal-visual-groups" aria-label="그룹 블록 배경">
    {runs.filter(run => run.slice.group.timeRule !== "ALL_DAY").map(run => {
      ...
      return <div key={`${group.id}:${slice.date}`} data-visual-group={group.id} data-group-date={slice.date} data-group-last-date={dates[last]} data-group-lane={slice.lane} className={`cal-visual-group ${selectedId === group.id ? "selected" : ""} ${allDay ? "all-day" : ""}`}
        ...
        <div className="cal-visual-group-heading">
          <button className="cal-group-drag" aria-label={`${group.title} 그룹 블록 열기`} title={label} onPointerDown={event => onPointerDown?.(event, group, slice, "move")} onClick={event => { event.stopPropagation(); if (!onPointerDown || event.detail === 0) onSelect(group, slice); }}>{label}</button>

        </div>
        {(["left", "right", "bottom"] as const).map(edge => <button key={edge} className={`cal-group-frame ${edge}`} aria-label={`${group.title} 그룹 ${edge} 테두리 선택`} onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); onSelect(group,slice); }} />)}
...
export function VisualGroupPeriodBands({ groups, dates = [], selectedId, onSelect, onPointerDown }: VisualGroupLayerProps) {
  ...
  return <div className="cal-group-periods" aria-label="그룹 기간" style={{ gridTemplateColumns: `repeat(${dates.length}, minmax(0, 1fr))` }}>
    {bands.map(run => {
      const { group } = run.slice, selected = selectedId === group.id;
      return <div key={`${group.id}:${run.first}`} data-group-band={group.id} data-band-start={dates[run.first]} data-band-end={dates[run.last]}
        ...
        <button title={visualGroupHeader(run, dates, true)} onClick={() => onSelect(group, run.slice)}>{visualGroupHeader(run, dates, true)}</button>
