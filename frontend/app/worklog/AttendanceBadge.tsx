...
// Exported so other Work Log components (e.g. TodayWorkPanel) can reuse the
// same status->color mapping instead of duplicating it.
...
export function AttendanceBadge({ status }: { status: AttendanceStatus }) {
  return (
    <span
      className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium leading-4 ${ATTENDANCE_STATUS_CLASSES[status]}`}
    >
      {status}
    </span>
  );
