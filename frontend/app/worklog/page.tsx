...
export default function WorkLogPage() {
  ...
  // Fixed recent-4-week dataset for the always-visible 근무 추이 trend
  // section (v2 trend-chart correction) — the Today-containing Monday–
  // Sunday week plus the three immediately preceding weeks, anchored to the
  // same `now` Today anchor used everywhere else on this page. Computed
  ...
  const [recentTrendRecords, setRecentTrendRecords] = useState<WorkLogRecord[]>(() => {
    ...
    const weekStarts = [addDays(todayWeekStart, -21), addDays(todayWeekStart, -14), addDays(todayWeekStart, -7), todayWeekStart];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-canvas-default">
      <div className="mx-auto flex w-full max-w-[1400px] flex-1 flex-col gap-4 px-6 py-4">
        ...
        {/* Independent of periodUnit (spec §2/§8): always visible below
            whichever record area is active, visually separated from it,
            and driven only by the fixed recent-4-week dataset above. */}
