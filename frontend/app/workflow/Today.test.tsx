...
async function main() {
  ...
  pass('Keyboard wiki autocomplete persists resolved stable identity');
  let releaseNote:()=>void=()=>{};const noteGate=new Promise<void>(resolve=>{releaseNote=resolve;});
  workflowApi.createNote=async title=>{await noteGate;return {...note,title,id:crypto.randomUUID()};};
  ...
  routes.length=0;await input(dateEditor,'Stream draft stays');saveFails=true;
  await click('Leave workpad');assert.deepEqual(routes,[]);assert.equal(dateEditor.value,'Stream draft stays');
  saveFails=false;await click('Leave workpad');assert.deepEqual(routes,['/worklog']);
  pass('Shell guard flushes all mounted stream and fixed editors');
  let batch=0;workflowApi.recordedDates=async()=>Array.from({length:3},(_,i)=>'2026-08-'+String(28-batch*3-i).padStart(2,'0'));
  ...
  pass('Practical mounted window retains focused drafts and reloads measured date placeholders');
  assert.ok(saves >= 3);
