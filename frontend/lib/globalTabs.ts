...
export function tabTarget(href: string): Omit<GlobalTab, "tabId" | "pinned"> | null {
  ...
  const authoring = path === "/authoring" || /^\/authoring\/(quick-motivation|recovery|reality|grounded-future|past|review|sexual-pattern)\/session\/[\da-f-]{36}(\/(full|report))?$/.test(path);
