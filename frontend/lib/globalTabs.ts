export type TabSystem = "WORK OS" | "NOTE SYS" | "LIFE CODE" | "DIET SYS" | "Calendar" | "WORK FLOW";
...
const keys: Record<TabSystem, string[]> = {
  ...
  "LIFE CODE": [], "DIET SYS": [], Calendar: ["date", "view", "mode"], "WORK FLOW": ["date", "block"],
};
export function tabTarget(href: string): Omit<GlobalTab, "tabId"> | null {
  ...
  const system: TabSystem | null = ["/workflow", "/workflow/projects", "/workflow/timeline", "/workflow/todo", "/workflow/today"].includes(path) ? "WORK FLOW" : ["/diet", "/diet/record", "/diet/planner", "/diet/progress"].includes(path) ? "DIET SYS" : path === "/notes" ? "NOTE SYS" : path === "/calendar" ? "Calendar" : path === "/life/categories" ? "LIFE CODE" : ["/worklog", "/worklog/checklist", "/worklog/attendance"].includes(path) ? "WORK OS" : null;
  ...
  const contextKey = system === "NOTE SYS" && query.has("note")
    ? `/notes?workspace=${query.get("workspace") ?? query.get("workspaceName") ?? ""}&note=${query.get("note")}` : route;
  const title = system === "WORK FLOW" ? `WORK FLOW · ${{"/workflow":"Today","/workflow/today":"Today","/workflow/projects":"Projects","/workflow/timeline":"Timeline","/workflow/todo":"To-do"}[path]}` : system === "DIET SYS" ? `DIET SYS · ${{"/diet":"홈","/diet/record":"기록","/diet/planner":"플래너","/diet/progress":"통계"}[path]}` : system === "LIFE CODE" ? "LIFE CODE · 카테고리" : system === "Calendar" ? `Calendar${query.get("date") ? ` · ${query.get("date")}` : ""}`
