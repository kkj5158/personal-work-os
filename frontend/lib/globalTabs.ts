export type TabSystem = "WORK OS" | "NOTE SYS" | "LIFE CODE" | "DIET SYS" | "Calendar" | "WORK FLOW" | "AUTHORING" | "CHECKLIST SYS";
...
const keys: Record<TabSystem, string[]> = {
  ...
  "LIFE CODE": [], "DIET SYS": [], Calendar: ["date", "view", "mode"], "WORK FLOW": ["date", "block", "note"], AUTHORING: [], "CHECKLIST SYS": ["identity", "area"],
};
export function tabTarget(href: string): Omit<GlobalTab, "tabId" | "pinned"> | null {
  ...
  const system: TabSystem | null = checklist ? "CHECKLIST SYS" : authoring ? "AUTHORING" : ["/workflow", "/workflow/projects", "/workflow/timeline", "/workflow/todo", "/workflow/today"].includes(path) ? "WORK FLOW" : ["/diet", "/diet/record", "/diet/planner", "/diet/progress", "/diet/gallery", "/diet/identity"].includes(path) ? "DIET SYS" : path === "/notes" ? "NOTE SYS" : path === "/calendar" ? "Calendar" : path === "/life/categories" ? "LIFE CODE" : ["/worklog", "/worklog/checklist", "/worklog/attendance"].includes(path) ? "WORK OS" : null;
