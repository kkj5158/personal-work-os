...
export function tabTarget(href: string): Omit<GlobalTab, "tabId"> | null {
  ...
  const title = system === "WORK FLOW" ? `WORK FLOW · ${{"/workflow":"Today","/workflow/today":"Today","/workflow/projects":"Projects","/workflow/timeline":"Timeline","/workflow/todo":"To-do"}[path]}` : system === "DIET SYS" ? `DIET SYS · ${{"/diet":"홈","/diet/record":"기록","/diet/planner":"플래너","/diet/progress":"통계","/diet/gallery":"갤러리","/diet/identity":"정체성/목표"}[path]}` : system === "LIFE CODE" ? "LIFE CODE · 카테고리" : system === "Calendar" ? `Calendar${query.get("date") ? ` · ${query.get("date")}` : ""}`
