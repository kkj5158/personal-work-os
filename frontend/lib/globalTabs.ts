...
export function tabTarget(href: string): Omit<GlobalTab, "tabId"> | null {
  ...
  const authoring = path === "/authoring" || /^\/authoring\/(recovery|reality|grounded-future)\/session\/[\da-f-]{36}(\/(full|report))?$/.test(path);
  const system: TabSystem | null = authoring ? "AUTHORING" : ["/workflow", "/workflow/projects", "/workflow/timeline", "/workflow/todo", "/workflow/today"].includes(path) ? "WORK FLOW" : ["/diet", "/diet/record", "/diet/planner", "/diet/progress"].includes(path) ? "DIET SYS" : path === "/notes" ? "NOTE SYS" : path === "/calendar" ? "Calendar" : path === "/life/categories" ? "LIFE CODE" : ["/worklog", "/worklog/checklist", "/worklog/attendance"].includes(path) ? "WORK OS" : null;
  ...
  const title = system === "AUTHORING" ? "AUTHORING" : system === "WORK FLOW" ? `WORK FLOW · ${{"/workflow":"Today","/workflow/today":"Today","/workflow/projects":"Projects","/workflow/timeline":"Timeline","/workflow/todo":"To-do"}[path]}` : system === "DIET SYS" ? `DIET SYS · ${{"/diet":"홈","/diet/record":"기록","/diet/planner":"플래너","/diet/progress":"통계"}[path]}` : system === "LIFE CODE" ? "LIFE CODE · 카테고리" : system === "Calendar" ? `Calendar${query.get("date") ? ` · ${query.get("date")}` : ""}`
