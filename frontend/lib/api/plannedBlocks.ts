...
import type { PlannedTimeBlock, CalendarPlanInput } from "./types";
...
export function createPlannedBlock(input: CalendarPlanInput): Promise<PlannedTimeBlock> {
  return apiClient.post<PlannedTimeBlock>("/api/planned-blocks", input);
}

export function updatePlannedBlock(id: string, input: CalendarPlanInput): Promise<PlannedTimeBlock> {
