...
test("challenge snapshot survives importance/active changes",()=>{const updated={...data,items:data.items.map(i=>({...i,importance:"OPTIONAL" as const,active:false}))};assert.equal(challengeProgress(base,updated,"2026-09-03").current,50);assert.equal(challengeProgress({...base,goalMode:"COUNT",targetValue:4},updated,"2026-09-03").progress,50);});
