...
export type DailyCheck = { date:string; itemId:string; state:"SUCCESS"|"FAILURE"|"MISSING"; memo:string };
export type Challenge = { id:string; title:string; type:"WEIGHT"|"CHECKLIST"|"MANUAL"; status:"WAITING"|"ACTIVE"|"COMPLETED"|"STOPPED"; startDate:string; endDate:string; color:string; keyPoint:string; notes:string[]; sortOrder:number; startWeight:number|null; targetWeight:number|null; itemIds:string[]; goalMode:"RATE"|"COUNT"; includeMissing:boolean; currentValue:number|null; targetValue:number|null };
export type WeightGoal = { id:string; challengeId:string|null; kind:"SHORT_TERM"|"WEEKLY"|"MONTHLY"|"FINAL"; date:string; value:number };
export type Milestone = { id:string; challengeId:string; date:string; value:number; title:string; memo:string };
export type ReferenceLine = { id:string; name:string; value:number; visible:boolean; goalKind?:WeightGoal["kind"] };
...
export type DietStore = { data:DietData; busy:boolean; error:string; save:<K extends keyof EntityMap>(kind:K,value:EntityMap[K])=>Promise<void>; remove:(kind:keyof EntityMap,id:string)=>Promise<void>; saveDay:(day:DailyRecord)=>Promise<void>; saveCheck:(check:DailyCheck)=>Promise<void>; saveSettings:(settings:DietSettings)=>Promise<void>; reorder:(kind:"items"|"challenges",ids:string[])=>Promise<void> };
