...
- Both `absenceAutoGenerated` and `absenceCorrectedAt` are exposed on
  `WorkRecordResponse` so a future frontend can distinguish "자동 결근,
  미정정" from "정정됨" for display.
- Uses the same optimistic-lock check (`expectedVersion`) as `upsert`.

## 12. Deferred

- Frontend integration — Work Log's frontend remains fully mock-backed,
  including the `결근 정정` UI itself (no such UI exists on the frontend
  today at all) — see `docs/project/work-log-roadmap.md`.
