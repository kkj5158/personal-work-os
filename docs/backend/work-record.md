...
## 13. Plan-aware reconciliation (attendance management batch)

`AbsenceBackfillService` (name/scheduler/config unchanged — see its own
class doc) now consults `AttendancePlanRepository` first for each missing
past date, before falling back to the original `workschedule`/`worksettings`
eligibility check:

1. A plan exists: `PAID_LEAVE`/`DAY_OFF` → confirmed as that same status
   (`WorkRecord.createReconciled`, `absenceAutoGenerated = false`);
   `WORK`/`HALF_DAY` → `ABSENT` (a no-show).
2. No plan at all → the original schedule-based check, unchanged.

`WorkRecord.createAbsence` now delegates to the generalized
`createReconciled(userId, workDate, status)`. `AbsenceRecordWriter.createIfMissing`
(renamed from `createAbsenceIfMissing`) is parameterized by the resolved
status but keeps its exact `REQUIRES_NEW`/re-check/unique-violation-race
idempotency guarantees. See `docs/product/work-attendance-management-design.md`
§3.

## 14. Future-actual-create block + historical create (attendance management batch)

`applyUpsert` rejects `upsert`/`PUT` when no record exists yet for a date
after today (`InvalidRequestException`) — a future date belongs to
`AttendancePlan`, never an actual row. Gated on `existing.isEmpty()` only:
an already-existing future row (there should be none in normal product use)
can still be edited/corrected, never permanently locked out.

The frontend's historical no-record create flow (a clickable 미입력 row/a
"근무 기록 생성" button on the Day view) needed no new endpoint — the
existing upsert already behaves as create-or-update; opening
`WorkLogRecordDetailModal` with a fresh `buildDraftRecord()` draft (the same
technique already used for "today before first save") creates nothing until
Save is clicked, and every normal validation in this document already
applies unchanged.
