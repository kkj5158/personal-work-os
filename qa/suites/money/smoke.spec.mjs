...
test('money.routes', async ({ page }) => {
  for (const [route, title] of [['', 'Home'], ['/transactions', 'Transactions'], ['/flow', 'Money Flow'], ['/review', 'Review Required'], ['/settings', 'Settings']]) {
    await expectApi(page, '/api/money/dashboard?', () => page.goto('/money' + route));
...
test('money.filters-reload', async ({ page }) => {
  ...
  const filtered = await expectApi(page, '/api/money/transactions?', () => page.getByPlaceholder('거래처 또는 메모').fill(query));
  expect(filtered.items).toEqual([]);
  await page.getByPlaceholder('거래처 또는 메모').fill('');
  ...
  for (const category of stored.filter(c => !c.archived)) await expect(page.getByLabel('카테고리', { exact: true }).locator('option', { hasText: category.name })).toHaveCount(1);
});

test('money.account-dialog', async ({ page }) => {
  await expectApi(page, '/api/money/account-balances', () => page.goto('/money/settings'));
  await page.getByRole('button', { name: '계좌 등록', exact: true }).last().click();
  await expect(page.getByRole('dialog')).toBeVisible();
  await page.getByRole('dialog').getByPlaceholder('생활비').fill('QA unsaved draft');
