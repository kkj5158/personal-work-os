...
public class WorkRecord implements Persistable<UUID> {
    ...
    public static WorkRecord createAbsence(UUID userId, LocalDate workDate) {
        return createReconciled(userId, workDate, WorkAttendanceStatus.ABSENT);
    }

    /**
     * Factory for the attendance-plan reconciliation catch-up — a fully
     * empty row for a past date that had no record at all, confirming
     * whatever status reconciliation decided (an outstanding plan's status,
     * or ABSENT for a no-show/no-plan date). Only {@code ABSENT} sets
     * {@link #absenceAutoGenerated}; confirming a planned PAID_LEAVE/DAY_OFF
     * is not an absence.
     */
    public static WorkRecord createReconciled(UUID userId, LocalDate workDate, WorkAttendanceStatus status) {
        WorkRecord record = new WorkRecord(userId, workDate);
        record.status = status;
        record.absenceAutoGenerated = status == WorkAttendanceStatus.ABSENT;
        return record;
    }
